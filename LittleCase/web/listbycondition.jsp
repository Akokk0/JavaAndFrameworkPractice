        ...
        <nav aria-label="Page navigation">
            <ul class="pagination">
                <c:if test="${(requestScope.pb.currentPage - 1) <= 0}">
                    <li class="disabled">
                        <a href="${pageContext.request.contextPath}/findUserByConditionWithPageServlet?currentPage=${requestScope.pb.currentPage - 1}&rows=10&name=${requestScope.condition.name[0]}&address=${requestScope.condition.address[0]}&email=${requestScope.condition.email[0]}" aria-label="Previous">
                ...
                <c:forEach begin="1" end="${requestScope.pb.totalPage}" var="i">
                    <c:if test="${requestScope.pb.currentPage == i}">
                        <li class="active"><a href="${pageContext.request.contextPath}/findUserByConditionWithPageServlet?currentPage=${i}&rows=10&name=${requestScope.condition.name[0]}&address=${requestScope.condition.address[0]}&email=${requestScope.condition.email[0]}">${i}</a></li>
                    </c:if>

                    <c:if test="${requestScope.pb.currentPage != i}">
                        <li><a href="${pageContext.request.contextPath}/findUserByConditionWithPageServlet?currentPage=${i}&rows=10&name=${requestScope.condition.name[0]}&address=${requestScope.condition.address[0]}&email=${requestScope.condition.email[0]}">${i}</a></li>
                    </c:if>
                </c:forEach>
                <c:if test="${(requestScope.pb.currentPage + 1) > requestScope.pb.totalPage}">
                    <li class="disabled">
                        <a href="${pageContext.request.contextPath}/findUserByConditionWithPageServlet?currentPage=${requestScope.pb.currentPage + 1}&rows=10&name=${requestScope.condition.name[0]}&address=${requestScope.condition.address[0]}&email=${requestScope.condition.email[0]}" aria-label="Next">
