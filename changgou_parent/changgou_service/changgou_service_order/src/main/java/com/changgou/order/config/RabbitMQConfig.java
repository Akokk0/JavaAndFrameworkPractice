...
public class RabbitMQConfig {
    ...
    //绑定添加积分任务队列与交换机
    @Bean
    public Binding CG_BUYING_ADDPOINT_Binding(@Qualifier(CG_BUYING_ADDPOINT) Queue queue, @Qualifier(EX_BUYING_ADDPOINTUSER) Exchange exchange) {
        return BindingBuilder.bind(queue).to(exchange).with(CG_BUYING_ADDPOINT_KEY).noargs();
    }
    //绑定添加积分任务完成队列与交换机
    @Bean
    public Binding CG_BUYING_FINISHADDPOINT_Binding(@Qualifier(CG_BUYING_FINISHADDPOINT) Queue queue, @Qualifier(EX_BUYING_ADDPOINTUSER) Exchange exchange) {
